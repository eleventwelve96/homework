document.addEventListener('DOMContentLoaded', () => {
    ...
    // SVG: зелёная галочка (точно как в вашем HTML)
    const verifiedSVG = `
    ...
    // Начинаем со второго отзыва (индекс 1 → Даниил В. — центральный)
    let currentIndex = 1;
    ...
    // Обновление трёх слотов
    const updateSlots = () => {
        const leftSlot = document.querySelector('.review-card-slot_position_left');
        const centerSlot = document.querySelector('.review-card-slot_position_center');
        const rightSlot = document.querySelector('.review-card-slot_position_right');
